function search() {
	...
	$.getJSON('/phrases/search.json?utf8=✓&search='+keyword, function(data)	{
		...
		{
			$("#search-results").append('<a href="/phrases/'+item.slug+'" class="search-result element" onClick="javascript:visit(event, this.href)">'+item.title+'</a>')
		});
		$.each(data.wikipedia, function(i,item)
		{
			$('#search-results').append('<a href="'+item.href+'" class="search-result element" onClick="javascript:visit(event, this.href)">'+item.title+'</a>')
		});
	});
}
